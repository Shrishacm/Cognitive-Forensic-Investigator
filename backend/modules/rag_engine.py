...
# ────────────────────────────────────────────────────────────────────────────
# System Prompt — CFI partner persona
# ────────────────────────────────────────────────────────────────────────────
...
- You are concise but never terse. You give thorough answers, not one-liners.
...
def _extract_investigator_leads(conversation_history: list) -> str:
    ...
    return (
        "Investigator's known leads and observations:\n"
        + "\n".join(leads[-3:])
        + "\n\n"
    )
...
def run_rag_query(
    ...
    """
    Optimized RAG pipeline with lean prompt context for fast Ollama response times.
    """
    start_time = time.time()

    # ── Step 1: Retrieve top 4 relevant evidence chunks ───────────────────
    chunks = []
    try:
    ...
    if case_info:
        ...
        file_list_str = ', '.join(c_files) if c_files else 'No files uploaded'
        case_summary_text = (
            f"Case: {c_name}" + (f" ({c_num})" if c_num else "") +
            f" | Status: {c_status}\n"
            f"Evidence: {file_list_str}\n\n"
        )

    # ── Step 4: Build evidence context (truncated for fast prompt eval) ───
    if chunks:
